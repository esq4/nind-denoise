...
class ImageProcessor:
    def __init__(self, config_path='./src/config/operations.yaml', verbose=False):
    ...
    def read_config(self, config_path='./src/config/operations.yaml', _nightmode=False, verbose=False) -> dict:
    ...
    def denoise_file(self, _args: dict, _input_path: pathlib.Path):
        ...
        sigma = int(_args['--sigma']) if '--sigma' in _args and _args['--sigma'] is not None else 1
        quality = str(int(_args['--quality'])) if '--quality' in _args and _args['--quality'] is not None else "90"
        iteration = str(int(_args['--iterations'])) if '--iterations' in _args and _args[
            '--iterations'] is not None else "10"
        verbose = bool(_args["--verbose"]) if "--verbose" in _args and _args["--verbose"] is not None else False
        ...
        cmd_gmic = _args["--gmic"] if "--gmic" in _args and _args["--gmic"] else \
            (os.path.join(os.path.expanduser("~\\"), "gmic-3.6.1-cli-win64\\gmic.exe") if os.name == "nt" \
                 else "/usr/bin/gmic")
        rldeblur = not (_args.get("--no_deblur", False) or not os.path.exists(cmd_gmic))

        # Check for darktable-cli
        cmd_darktable = _args["--dt"] if "--dt" in _args and _args["--dt"] else \
        ...
        good_file = self.check_good_input(_input_path, valid_extensions) or self.check_good_input(input_xmp, '.xmp')
        if not good_file:
        ...
        while outpath.exists():
            ...
            i += 1
            if i >= 99:
                print("\nError: too many files with the same name already exist. Check:", outpath.parent)
                raise FileExistsError

        self.parse_darktable_history_stack(input_xmp, config=self.config, verbose=verbose)
        ...
        model_config = self.config["models"]["nind_generator_650.pt"]
        ...
        self.clone_exif(_input_path, stage_one_denoised_filepath)
        ...
        self.clone_exif(stage_one_output_filepath, outpath, verbose=verbose)

        # ========== clean up ==========
        if not _args.get('--debug', False):
    ...
    if input_path.is_dir():
        for file in input_path.iterdir():
            if file.suffix.lower() in valid_extensions:
                print("\n-----------------------", file.name, "-------------------------\n")
                processor.denoise_file(dict(args), _input_path=file)
    else:
        processor.denoise_file(dict(args), _input_path=input_path)
