...
TARGET_DPATH = os.path.join('/media/a/vxd1', 'datasets', 'NIND')
...
def get_img(bname, isoval, ext, attempts_left, datelimit, use_wget, custom_program = 'curl'):
    def get_latest_reqimg_info(imname, datelimit):
        ...
        # Заголовок User-Agent, имитирующий Firefox (требование API)
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:90.0) Gecko/20100101 Firefox/90.0"}
        #headers = {'user-agent': 'NIND-download-script/0.0.1'}
        request = session.get(url=apiurl, params=rparams, headers=headers)
        time.sleep(1)
        try:
    ...
    def checkfile(path, reqsha1):
        ...
        global count
        count = count + 1
        print('%s : Validated %s'%(count, path))
        return True
    ...
    if not checkfile(fpath, reqsha1):
        url = imageinfo['url']
