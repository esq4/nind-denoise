...
parser.add_argument(
    "--stride", default=STRIDE, type=int, help=f"Useful crop size (default: {STRIDE})"
)
parser.add_argument(
    "--dsdir",
    default=FS_DS_DPATH,
    type=str,
    help=f"Input (full-size) dataset directory. Default: {FS_DS_DPATH}",
)
parser.add_argument(
    "--resdir",
    default=CROPPED_DS_DPATH,
    type=str,
    help=f"Output cropped dataset directory ([resdir]/dsdir_[cs]_[stride]). Default: {CROPPED_DS_DPATH}",
)
parser.add_argument(
    "--max_threads",
    default=math.ceil(cpu_count() / 2),
    type=int,
    help=f"Maximum number of active threads, default={math.ceil(cpu_count()/2)}",
)
args = parser.parse_args()

dsdir = utilities.get_leaf(args.dsdir)
resdir = os.path.join(
    utilities.get_root(args.dsdir),
    "cropped",
    dsdir + "_" + str(args.cs) + "_" + str(args.stride),
)
todolist = []
