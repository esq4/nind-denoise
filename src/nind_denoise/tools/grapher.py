...
parser.add_argument(
    "--res_dir",
    default=os.path.join(RES_DPATH, "test"),
    type=str,
...
parser.add_argument(
    "--save_dir",
    default=os.path.join(RES_DPATH, "graphs", "auto"),
    type=str,
    help="Where graphs are saved",
)
...
parser.add_argument(
    "--components",
    nargs="+",
    help="space-separated line components (eg Noisy NIND Artificial BM3D) or keywords (eg: p2p)",
)
...
parser.add_argument(
    "--run", default=None, type=int, help="Generate a single graph number"
)
...
if args.mode == "std":
    if args.components:
        ...
        components = [
            ...
            "SIDD (U-Net)",
            "BM3D",
            "NIND:X-T1+C500D (U-Net)",
            "NIND:X-T1+C500D + SIDD (U-Net)",
            "NIND:X-T1 ISO6400-only (U-Net)",
            "Artificial noise on NIND:X-T1 (U-Net)",
            "Reconstruct noise on NIND:X-T1 (U-Net)",
            "NIND:X-T1 (Red-Net)",
        ]
else:
    # components = ['GT']
    ...
    for experiment in os.listdir(args.res_dir):
        if args.mode == "all":
            if (
                experiment == "GT"
                or experiment == "Noisy input"
                or "bm3d" in experiment
            ):
                continue
    ...
    for respath in data[component]["resfiles"]:
        with open(respath) as f:
            for res in csv.reader(f):
                ...
                if newssim > oldssim:
                    data[component]["results"][image][isoval] = {
                        "ssim": float(res[1]),
                        "mse": float(res[2]),
                        "exp": respath.split("/")[-2],
                    }
...
for image in images:
    ...
    for component in components:
        try:
            ssimscore = [
                data[component]["results"][image][iso][args.metric] for iso in isos
            ]
