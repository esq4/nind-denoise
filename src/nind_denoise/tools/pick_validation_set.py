...
parser.add_argument(
    "--overwrite", action="store_true", help="Overwrite existing validation set?"
)
        ...
        for base_iso in base_isos:
            ...
            for noisy_iso in isos:
                ...
                for crop_fn in crops_filenames:
                    crops_paths.append(
                        [
                            os.path.join(base_iso_dpath, crop_fn),
                            os.path.join(
                                noisy_iso_dpath, crop_fn.replace(base_iso, noisy_iso)
                            ),
                        ]
                    )
...
res_fpath = os.path.join(
    "configs",
    f'validation_set_{args.num_crops}_{"+".join(train_data_dnames)}_{os.path.basename(os.path.relpath(test_reserve_str))}',
)
