...
def get_img(
    bname, isoval, ext, attempts_left, datelimit, use_wget, custom_program=None
):
    def get_latest_reqimg_info(imname, datelimit):
        ...
        try:
            imageinfo = next(iter(request.json()["query"]["pages"].values()))[
                "imageinfo"
            ][0]
            return imageinfo
        except:
        ...
        if use_wget:
            ...
            with open(path, "wb") as f:
                response = requests.get(
                    url, headers={"user-agent": "NIND-download-script/0.0.1"}
                )
    ...
    while not checkfile(fpath, reqsha1):
        if attempts_left == 0:  # negative max_attempts -> unlimited attempts
            return dlerrors.append(
                "Error: Unable to download %s (source: %s)" % (fpath, url)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=last_update,
        type=str,
        help="Latest date of upload, used to get a specific version of the dataset (default: %s)"
        % (last_update),
    )
    parser.add_argument(
        "--use_wget",
        action="store_true",
        help="Use wget instead of python's request library (more likely to succeed)",
    )
    parser.add_argument(
        "--custom_program",
        help="Custom program (alternative to wget), must follow the pattern custom_program url -O path",
    )
    parser.add_argument(
        "--target_dir",
        default=TARGET_DPATH,
        type=str,
        help=f"Target directory (default: {TARGET_DPATH})",
    )
    parser.add_argument(
        "--sets2dl",
        nargs="*",
        help="Space separated list of image sets to download (default: %s)"
        % (" ".join(imageslist.keys())),
    )
    parser.add_argument(
        "--max_attempts",
        default=3,
    ...
    if sum(["Unable to download" in error for error in dlerrors]) > 0:
        dlerrors.append(
            "Some errors were encountered and corrupted files may be present, you should remove them manually or run this script again."
        )
