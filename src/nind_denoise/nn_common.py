...
class Model:
    def __init__(self, save_dict=True, device=torch.accelerator.current_accelerator(), printer=None, debug_options=[]):
        if printer is None:
            self.print = print
        else:
...
class Discriminator(Model):
    def __init__(self, models_dpath, beta1, lr, patience, network='Hul112Disc', weights_dict_path=None,
                 model_path=None, device=torch.accelerator.current_accelerator(), loss_function='MSE',
