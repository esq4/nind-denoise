...
from nind_denoise.libs.brummer2019.networks.nnModules import (
    Hul112Disc,
    Hulb112Disc,
    Hulb128Net,
    Hulbs128Net,
...
def get_scheduler(optimizer, opt, generator):
    ...
    if opt.lr_policy == "plateau":
        schedulerSSIM = lr_scheduler.ReduceLROnPlateau(
            optimizer,
            factor=opt.lr_gamma,
            verbose=True,
            threshold=1e-8,
            patience=opt.lr_step_size,
        )
...
def update_learning_rate(
    scheduler, optimizer, loss_avg=None, scheduler_name="untitled scheduler"
):
...
def init_net(
    ...
    init_type="normal",
    init_gain=0.02,
    gpu_id=torch.accelerator.current_accelerator(),
):
...
def define_D(
    ...
    if netD == "basic":
        ...
        net = Hulb112Disc(
            input_nc, out_activation=out_activation, finalpool=finalpool, funit=funit
        )
