...
from nind_denoise.libs.common import libimganalysis, np_imgops, pt_helpers, pt_ops, utilities
...
class DenoisingDataset(torch.utils.data.Dataset):
    ...
    def get_x_y_paths(self, index):
        ...
        xpath = os.path.join(
            img[0].replace("ISOBASE_", xchoice + "_").replace("/ISOBASE/", "/" + xchoice + "/")
        )
        ychoice = random.choice(img[2])
        ypath = os.path.join(
            img[0].replace("ISOBASE_", ychoice + "_").replace("/ISOBASE/", "/" + ychoice + "/")
        )
        return xpath, ypath
            ...
            for biso in el[1]:
                for noisy_iso in el[2]:
                    gt_crop_path = os.path.join(
                        el[0].replace("ISOBASE_", biso + "_").replace("/ISOBASE/", "/" + biso + "/")
                    )
