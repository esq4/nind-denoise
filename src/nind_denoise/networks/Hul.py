...
class Hulb128Net(nn.Module):
    ...
    def forward(self, x):
        ...
        l118 = torch.cat(
            [
                self.enc122to118std(l122),
                self.enc122to118dil(l122),
                self.enc128to118dil(x),
            ],
            1,
        )
        del x
        ...
        del l38
        l118 = torch.cat(
            [l118, self.dec114to118std(l114), self.dec114to118dil(l114)], 1
        )
        del l114
        l122 = torch.cat(
            [l122, self.dec118to122std(l118), self.dec118to122dil(l118)], 1
        )
        del l118
        l126 = torch.cat(
            [l126, self.dec122to126std(l122), self.dec122to126dil(l122)], 1
        )
...
class Hul112Disc(nn.Module):
    def __init__(
        self, input_channels=3, funit=32, finalpool=False, out_activation="PReLU"
    ):
...
class Hulf112Disc(nn.Module):
    def __init__(
        self, input_channels=6, funit=32, finalpool=False, out_activation="PReLU"
    ):
