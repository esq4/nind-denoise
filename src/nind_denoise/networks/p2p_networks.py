...
def init_net(net, init_type='normal', init_gain=0.02, gpu_id=torch.accelerator.current_accelerator()):
    net.to(torch.accelerator.current_accelerator())
    #init_weights(net, init_type, gain=init_gain)
    return net


def define_G(input_nc, output_nc, ngf, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_id=torch.accelerator.current_accelerator(), net_type='Resnet'):
    net = None
...
def define_D(input_nc, ndf, netD,
             n_layers_D=3, norm='batch', use_sigmoid=False, init_type='normal', init_gain=0.02, gpu_id=torch.accelerator.current_accelerator(), out_activation=None, finalpool = False, funit=32):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
