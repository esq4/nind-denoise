...
# Load the RL module from src using dynamic import (following project guidelines)
path = str(
    p.Path(__file__).resolve().parents[1]
    / "src"
    / "nind_denoise"
    / "rl_pt"
    / "richardson_lucy_deconvolution.py"
)
loader = importlib.machinery.SourceFileLoader("rl_local", path)
spec = importlib.util.spec_from_loader(loader.name, loader)
mod = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = mod
loader.exec_module(mod)
richardson_lucy_gaussian = mod.richardson_lucy_gaussian
...
def test_richardson_lucy_gaussian_with_uint8():
    ...
    image = torch.tensor(
        [
            [[255, 255], [127, 127], [63, 63]],
            [[191, 191], [127, 127], [63, 63]],
            [[127, 127], [191, 191], [255, 255]],
        ],
        dtype=torch.uint8,
    )
